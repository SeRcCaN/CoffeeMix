...
class MochaViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NetworkServices.shared.getAllCoffeeRecipes(url: MOCHA_URL) { (coffees) in
            self.mochas = coffees
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetail"{
            ...
            destination?.selectedCoffee = mochas[indexPath.row]
...
extension MochaViewController: UIViewControllerTransitioningDelegate{
...
extension MochaViewController: UITableViewDelegate, UITableViewDataSource{
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let mocha = mochas[indexPath.row]
        cell.configureCell(coffee: mocha)
